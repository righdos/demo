   ...
   /* $(".title_list .pen").click(function () {
    ...
    function jiazai(){
        if(getUrlParams("type")){
            ...
            xhr.open("GET","test.php?number="+key);
            xhr.send(null);
            xhr.onreadystatechange=function(){
                if(xhr.readyState==4){
                    if(xhr.status==200){
                        document.getElementById("chaxun").innerHTML=xhr.responseText
                    }
                }
            }
            ...
            $("#cover").attr("src",retule.coverImg);
            $("#articleTitle").text(retule.title);
            $("#updateTime").text(retule.updateAt);
            $("#content").html(retule.content)
        }
    }*/
